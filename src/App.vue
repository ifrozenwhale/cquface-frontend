<template>
  <v-app>
    <v-navigation-drawer v-model="drawer" :clipped="$vuetify.breakpoint.lgAndUp" :mini-variant.sync="mini" app>
      <v-list dense>
        <v-list-item v-for="(item, index) in items" :key="item.text" link @click="test(index)">
          <v-list-item-icon>
    ...
    <v-app-bar :clipped-left="$vuetify.breakpoint.lgAndUp" app color="blue darken-3" dark>
      ...
      <!-- if not login -->
      <v-btn icon>
        <v-icon large v-if="ok" @click="logout">mdi-logout</v-icon>
      </v-btn>
      <v-dialog v-model="dialog" max-width="290">
        <v-card>
          ...
          <v-card-actions>
            ...
            <v-btn color="light-blue darken-3" class="red-text" @click="dialog = false">
              <span class="white--text text--lighten-2"> 取消</span>
            </v-btn>
            <v-btn color="light-blue darken-4" @click="logoutConfirm">
      ...
      <v-dialog v-model="starDialog" max-width="290">
        <v-card>
          <v-card-title class="headline text-center">
            欢迎给我们Star！
          </v-card-title>
          ...
          <v-card-actions>
            ...
            <v-btn color="light-blue darken-3" class="red-text" @click="starDialog = false">
              <span class="white--text text--lighten-2"> 取消</span>
            </v-btn>
            <v-btn color="light-blue darken-4" @click="starConfirm()">
              <span class="white--text text--lighten-2">确认</span>
            </v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
      <v-btn icon large @click="gocqu()">
        <v-avatar size="35px" item>
          <v-img src="./assets/cqu.png" alt="Chongiqng University"></v-img>
        </v-avatar>
      </v-btn>
    </v-app-bar>
