<template>
  ...
  <v-app>
    ...
    >
      ...
      <v-col md="20">
        <h1
          style="color:#757575 ; font-size:50px"
          @click="refresh"
        >
          动态详情
        </h1>
...
export default {
  ...
  methods: {
    refresh() {
      ...
      console.log(this.user_id); // 照片主人的user_id
      getReportDetail(
        this.user_id,
        this.photo_id,
        ...
        this.avator = response.data.portrait;
        this.likeNum = response.data.favorite_num;
        this.commentNum = response.data.comment_num;
        this.isfavorite = response.data.is_favorites;
        this.commentItems = [];
        response.data.comments_info.forEach(element => {
          this.commentItems.push({
            name: element.name,
            cont: element.cont,
            avator: element.portrait
          });
        });
    ...
    like() {
      ...
      star(localStorage.getItem("account"), this.photo_id).then(response => {
